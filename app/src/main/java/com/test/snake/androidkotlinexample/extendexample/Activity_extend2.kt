...
import kotlinx.android.synthetic.main.activity_extend2.*

class Activity_extend2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_extend2)
        ...
        when (requestCode) {
            RequestPermissionCode -> if (grantResults.size > 0) {
                ...
                if (StoragePermission && RecordPermission) {
                    Toast.makeText(this@Activity_extend2, "Permission 허락됨", Toast.LENGTH_LONG).show()
                } else {
                    Toast.makeText(this@Activity_extend2, "Permission 거부됨", Toast.LENGTH_LONG).show()
    ...
    /*
    미션 1: 음성파일 play 중 stop할 수 있는 기능만들기(버튼1개 누르면 음성play 중 멈춤)
    미션 2: 저장된 음성파일을 공유하기(버튼 1개 누르면 공유하기)
    */
}
